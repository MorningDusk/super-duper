...
def run_vod_to_shop(image, direction):
   ...
   # step1_myeongpum의 category_sub
    ...
    final_return_list = [step1_myeongpum.get("product_code")]
    for score_idx, p_idx, b_idx in reordered_mapping[:4]:
        product = dupe_products[p_idx]
        print("---")
        print("추천 상품명:", product.get("product_name"))
        print("카테고리:", product.get("category_sub"))
        print("추천 상품 코드:", product.get("product_code"))
        # print("최종 가격:", product.get("final_price"))
        print(f"점수 (GDP 적용): {weighted_scores[(score_idx, p_idx, b_idx)]:.4f}")
        print("---")
        final_return_list.append(product.get("product_code"))
    # print(final_return_list)
    return final_return_list

# if __name__ == "__main__":
#     QUERY_IMAGE_PATH = "static/local_data/highlight_image/highlight_0001_0001.png"
#     print(run_vod_to_shop(QUERY_IMAGE_PATH, "middle"))
